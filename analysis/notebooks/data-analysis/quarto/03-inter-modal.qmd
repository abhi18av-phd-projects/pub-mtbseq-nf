...
## (Inter-mode) Samples with non-zero differences `MTBseq-nf` (default) vs `MTBseq` (standard)
...
ranges_default_standard <- foreach(i=1:length(numericVars), .combine = cbind) %do% {
...
names(ranges_default_standard) <- numericVars

ranges_default_standard.summary <- apply(ranges_default_standard,2, function(x) sum(x!=0))


parameters.of.interest <- ranges_default_standard.summary[ranges_default_standard.summary > 0 ]

table_standard_default <-  sampleids |>
      bind_cols(ranges_default_standard) |>
...
ranges_default_standard.summary <- apply(ranges_default_standard,2, function(x) sum(x!=0))
interStandarddefaultVariants <- ranges_default_standard.summary[which(ranges_default_standard.summary > 0)]


# Prepare long-format data for faceting (following your standard)
ranges_default_standard_long <- ranges_default_standard |>
  bind_cols(SampleID = sampleids$SampleID) |>
  select(SampleID, all_of(names(interStandarddefaultVariants))) |>
  pivot_longer(cols = -SampleID, names_to = "Parameter", values_to = "Difference") |>
  filter(Difference > 0) |>
  mutate(SampleID = factor(SampleID))

# Single faceted plot
ggplot(ranges_default_standard_long, aes(x = SampleID, y = Difference)) +
  geom_col(alpha = 0.7) +
  facet_wrap(~Parameter, scales = "free_y", ncol = 2) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(
    title = "Inter-modal variations in MTBseq vs MTBseq-nf (default)",
    x = "Sample ID",
    y = "Difference (SNPs)",
...
upset_data <- ranges_parallel_standard |>
  select(c(names(parameters.of.interest))) %>%
  filter(rowSums(.) > 0) |>
  mutate(across(everything(), function(x) ifelse(x > 0, 1, 0))) 
